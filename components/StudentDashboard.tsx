...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating }) => {
  ...
  useEffect(() => {
    const config = settings?.featurePopup;
    ...
    const checkPopup = () => {
      ...
      if (isSubscribed && user.subscriptionEndDate) {
          ...
          // Show Expiry Popup if within range (e.g. 48 hours)
          if (hoursLeft > 0 && hoursLeft <= (config.showNearExpiryHours || 48)) {
              // Only show if enough time passed since last popup
              if ((now - lastPopupTime) > intervalMs) {
                  setShowExpiryPopup(true);
                  setLastPopupTime(now);
                  localStorage.setItem(`nst_last_feature_popup_${user.id}`, now.toString());
                  return; // Don't show feature popup if showing expiry
              }
          }
      }

      // 2. Check Feature Popup (For Free Users or Configured)
      let shouldShowFeature = !isSubscribed;
      if (config.showToPremiumUsers && isSubscribed) shouldShowFeature = true;

      if (shouldShowFeature && (now - lastPopupTime) > intervalMs) {
  ...
  }, [user.id, user.isPremium, user.subscriptionEndDate, settings?.featurePopup, lastPopupTime]);
              ...
              <div className="space-y-6 pb-24">
                      ...
                      {settings?.featuredItems && settings.featuredItems.length > 0 && (
                          <div className="px-1 mb-2">
                              <h3 className="font-black text-slate-800 flex items-center gap-2 mb-3 px-1">
                                  <Sparkles className="text-yellow-500" size={18} /> {settings.featuredSectionTitle || 'Recommended for You'}
                              </h3>
                  ...
                  <div>
                      ...
                      <div className="grid grid-cols-4 gap-3">
                          ...
                          {/* NEW BUTTONS */}
                          <button onClick={() => setShowFeaturesModal(true)} className="aspect-square bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center gap-1 hover:bg-slate-50 transition-all group">
                          ...
                          {/* SESSION SEARCH */}
                          <button onClick={() => setShowSessionSearch(true)} className="aspect-square bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center gap-1 hover:bg-slate-50 transition-all group">
                              <div className="bg-teal-50 p-2 rounded-full group-hover:bg-teal-100 transition-colors"><ListChecks size={20} className="text-teal-600" /></div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
                ...
                <div className="bg-white rounded-3xl w-full max-w-sm max-h-[80vh] flex flex-col shadow-2xl overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-4">
                        ...
                        <div className="grid gap-2">
                            {/* USE ALL FEATURES LIST */}
                            {ALL_APP_FEATURES.map((feat, i) => (
                                <div key={feat.id} className="flex items-center gap-3 p-2 border-b border-slate-100 last:border-0 hover:bg-slate-50 rounded-lg transition-colors">
        ...
        <FeatureComparisonModal 
            ...
            freeFeatures={settings?.featurePopup?.freeFeatures || ['Basic MCQs', 'Standard PDFs', 'Limited Videos']}
            premiumFeatures={settings?.featurePopup?.premiumFeatures || ['Ultra Video Access', 'Premium PDFs', 'Weekly Tests', 'No Ads']}
            onUpgrade={() => {
        ...
        {showSessionSearch && (
            <SessionView 
                ...
                classLevel={user.classLevel || '10'} 
                onSelectTopic={async (res) => {
                    setShowSessionSearch(false);
                    const subjectList = getSubjectsList(user.classLevel || '10', user.stream || null);
                    const subject = subjectList.find(s => s.name === res.subject);
                    ...
                    if (subject) {
                        ...
                        try {
                            ...
                            // 2. Fuzzy Match Topic to Chapter
                            const matchedChapter = chapters.find(c => 
                                c.title.toLowerCase().includes(res.topic.toLowerCase()) || 
                                res.topic.toLowerCase().includes(c.title.toLowerCase())
                            );

                            // 3. Set State to Open Chapter
                            setSelectedSubject(subject);
                            setChapters(chapters); // Populate list
                            setContentViewStep(matchedChapter ? 'PLAYER' : 'CHAPTERS'); // Go to player if found, else list
                            if (matchedChapter) {
                                setSelectedChapter(matchedChapter);
                                setFullScreen(true);
                                showAlert(`Auto-opened "${matchedChapter.title}"`, 'SUCCESS');
                            } else {
                                showAlert(`Topic "${res.topic}" found in ${res.subject}. Please select the specific chapter.`, 'INFO');
                            }
                        } catch (e) {
                            console.error(e);
                            showAlert("Error loading session content.", 'ERROR');
                        } finally {
                            setIsLoadingContent(false);
                        }
                    } else {
                        showAlert(`Subject ${res.subject} not available in your stream.`, 'ERROR');
                    }
                }} 
            />
        )}
